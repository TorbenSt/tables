<x-layouts.app title="Dashboard">
    ...
    <div class="grid gap-6 md:grid-cols-2">
        ...
        <section class="rounded-xl border border-stone-200 bg-white p-6 shadow-sm">
            ...
            <p class="mt-2 text-sm text-stone-600">
                Adresse oder Koordinaten → Satellitenbild → Außentische per Klick markieren.
                Sonnenstand plus OSM-Gebäude und Bäume ergeben die Jahres-Sonnenzeiten.
            </p>
            <div class="mt-4 flex flex-wrap gap-2">
                <a href="{{ route('map-sites.create') }}" class="rounded-md bg-stone-900 px-3 py-2 text-sm text-white hover:bg-stone-700">Tische markieren</a>
                <a href="{{ route('map-sites.index') }}" class="rounded-md border border-stone-300 px-3 py-2 text-sm hover:bg-stone-50">Standorte</a>
                <a href="{{ route('photo-sessions.index') }}" class="rounded-md border border-stone-300 px-3 py-2 text-sm hover:bg-stone-50">Alter Foto-Flow</a>
            </div>
            <dl class="mt-5 grid grid-cols-2 gap-3 text-sm">
                <div class="rounded-lg bg-stone-50 p-3">
                    <dt class="text-stone-500">Kartenstandorte</dt>
                    <dd class="text-lg font-semibold">{{ $mapSites }}</dd>
                </div>
                <div class="rounded-lg bg-stone-50 p-3">
                    <dt class="text-stone-500">Markierte Tische</dt>
                    <dd class="text-lg font-semibold">{{ $mapTables }}</dd>
                </div>
            </dl>
        </section>
    </div>
